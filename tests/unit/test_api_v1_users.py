...
class TestApiV1UsersUnit(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_update_user_self_no_admin_change(self):
        ...
        data = UserPut(name="New Name", admin=True) # User tries to make themselves admin
        mock_request = MagicMock()
        
        await self.controller.update(user_id="_self", request=mock_request, data=data, fields=set())
        
        # Verify data.admin was set to None for _self update
        self.assertIsNone(data.admin)
        self.mock_crud_users.update.assert_called_once_with(_id="my-id", payload=data, fields=[])
    ...
    async def test_create_user_admin_required(self):
        ...
        mock_request = MagicMock()
        await self.controller.create(user_id="newuser", request=mock_request, data=data, fields=set())
        
        self.mock_authorize.require_admin.assert_called_once_with(request=mock_request)
        self.mock_crud_users.create.assert_called_once_with(_id="newuser", payload=data, fields=[])
    ...
    async def test_delete_user(self):
        ...
        self.mock_crud_creds.delete_all_from_owner.assert_called_once_with(owner="user1")
        self.mock_crud_teams.delete_user_from_teams.assert_called_once_with(user_id="user1")
    ...
    async def test_update_user_admin(self):
        ...
        mock_request = MagicMock()
        await self.controller.update(user_id="user1", request=mock_request, data=data, fields=set())
        
        self.mock_authorize.require_admin.assert_called_once_with(request=mock_request)
        self.mock_crud_users.update.assert_called_once_with(_id="user1", payload=data, fields=[])
