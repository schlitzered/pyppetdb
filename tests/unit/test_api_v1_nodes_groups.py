...
class TestApiV1NodesGroupsUnit(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_create_group_calls_upsert(self):
        ...
        await self.controller.create(node_group_id="group1", request=mock_request, data=data, fields=set())
    ...
    async def test_delete_group(self):
        ...
        self.mock_authorize.require_admin.assert_called_once_with(request=mock_request)
        self.mock_crud_nodes.delete_node_group_from_all.assert_called_once_with(node_group_id="group1")
    ...
    async def test_get_group(self):
        ...
        mock_request = MagicMock()
        await self.controller.get(node_group_id="group1", request=mock_request, fields=set())
    ...
    async def test_update_group(self):
        ...
        data = NodeGroupUpdate(teams=["team1"])
        mock_request = MagicMock()
        await self.controller.update(node_group_id="group1", request=mock_request, data=data, fields=set())
