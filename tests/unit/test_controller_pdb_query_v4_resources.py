...
class TestControllerPdbQueryV4ResourcesUnit(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.controller = ControllerPdbQueryV4Resources(self.log, self.mock_config, self.mock_auth_cert)
    ...
    async def test_get(self):
        ...
        mock_response.json.return_value = [{"certname": "node1", "type": "Class", "title": "Main"}]
        
        # We need to mock the http property or the AsyncClient it returns
        with patch.object(ControllerPdbQueryV4Resources, "http", new_callable=PropertyMock) as mock_http_prop:
        ...
        self.assertEqual(result, [{"certname": "node1", "type": "Class", "title": "Main"}])
