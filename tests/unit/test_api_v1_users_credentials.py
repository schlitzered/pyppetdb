...
class TestApiV1UsersCredentialsUnit(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_create_credential_self(self):
        ...
        await self.controller.create(data=payload, user_id="_self", request=mock_request)
        
        self.mock_authorize.get_user.assert_called_once_with(request=mock_request)
        self.mock_crud_creds.create.assert_called_once_with(owner="real-user-id", payload=payload)
    ...
    async def test_create_credential_other_admin(self):
        ...
        await self.controller.create(data=payload, user_id="other-user", request=mock_request)
        
        self.mock_authorize.require_admin.assert_called_once_with(request=mock_request)
        self.mock_crud_creds.create.assert_called_once_with(owner="other-user", payload=payload)
    ...
    async def test_delete_credential_self(self):
        ...
        mock_request = MagicMock()
        await self.controller.delete(user_id="_self", credential_id="cred1", request=mock_request)
    ...
    async def test_delete_credential_other_admin(self):
        ...
        mock_request = MagicMock()
        await self.controller.delete(user_id="other", credential_id="cred1", request=mock_request)
    ...
    async def test_get_credential_self(self):
        ...
        mock_request = MagicMock()
        await self.controller.get(user_id="_self", credential_id="cred1", request=mock_request, fields=set())
        
        self.mock_crud_creds.get.assert_called_once_with(owner="my-id", _id="cred1", fields=[])
    ...
    async def test_update_credential_self(self):
        ...
        await self.controller.update(
            user_id="_self", credential_id="cred1", data=payload, request=mock_request, fields=set()
        )
