...
class TestCrudNodesSecretsRedactorUnit(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        with patch(
            "pyppetdb.crud.nodes_secrets_redactor.CrudNodesSecretsRedactorCache"
        ) as mock_cache_class:
            self.mock_cache = mock_cache_class.return_value
            self.crud = CrudNodesSecretsRedactor(
...
class TestCrudNodesSecretsRedactorCacheUnit(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.cache = CrudNodesSecretsRedactorCache(
            self.log, self.mock_coll, self.mock_redactor
        )
