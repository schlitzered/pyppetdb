...
class TestApiV1TeamsUnit(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_update_team(self):
        self.mock_authorize.require_admin = AsyncMock()
        self.mock_crud_teams.get = AsyncMock(return_value=TeamGet(id="team1", ldap_group="old"))
    ...
    async def test_validate_permissions_success(self):
        ...
        await self.controller._validate_permissions(["CA:SPACES:CREATE", "CA:AUTHORITIES:UPDATE"])
        
        # Test granular permissions with resource lookup
        self.mock_crud_ca_spaces.get = AsyncMock(return_value=MagicMock(id="space1"))
        self.mock_crud_ca_authorities.get = AsyncMock(return_value=MagicMock(id="auth1"))
        ...
        self.mock_crud_ca_authorities.get.assert_called_once_with("auth1", fields=["id"])

    async def test_validate_permissions_invalid_format(self):
        with self.assertRaises(QueryParamValidationError) as cm:
            await self.controller._validate_permissions(["CA:SPACES:VIEW"]) # VIEW is now invalid
    ...
    async def test_validate_permissions_resource_not_found(self):
        ...
        with self.assertRaises(QueryParamValidationError) as cm:
            await self.controller._validate_permissions(["CA:SPACES:space1:CERTS:UPDATE"])
        self.assertIn("CA Space 'space1' does not exist", str(cm.exception.detail))
        
        self.mock_crud_ca_authorities.get = AsyncMock(side_effect=ResourceNotFound("auth1"))
        with self.assertRaises(QueryParamValidationError) as cm:
            await self.controller._validate_permissions(["CA:AUTHORITIES:auth1:CERTS:UPDATE"])
