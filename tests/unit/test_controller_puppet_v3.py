...
class TestControllerPuppetV3Unit(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_catalog_post_not_cached(self):
        ...
        facts_json = json.dumps(
            {"values": {"os": {"family": "RedHat"}, "ipaddress": "1.2.3.4"}}
        )
        ...
        self.assertEqual(
            kwargs["facts"], {"os.family": "RedHat", "ipaddress": "1.2.3.4"}
        )
    ...
    async def test_file_content_get_module(self):
        controller = ControllerPuppetV3FileContent(
    ...
    async def test_file_content_get_plugins(self):
        ...
        with patch("pyppetdb.controller.puppet.v3.file_content.Path") as mock_path:
            ...
            mock_path.return_value = mock_path
            mock_path.__truediv__.side_effect = lambda x: (
                mock_env_modules if x == "modules" else mock_path
            )
            mock_module_dir.__truediv__.return_value = mock_module_dir
            mock_module_dir.__truediv__.side_effect = lambda x: (
    ...
    async def test_facts_put(self):
        controller = ControllerPuppetV3Facts(
            self.log, self.mock_config, self.mock_http, self.mock_auth_cert
        )
        mock_request = MagicMock()
        facts_data = {
    ...
    async def test_node_get(self):
        controller = ControllerPuppetV3Node(
            self.log, self.mock_config, self.mock_http, self.mock_auth_cert
        )
        mock_request = MagicMock()
        mock_request.query_params = {}
    ...
    async def test_report_put(self):
        controller = ControllerPuppetV3Report(
            self.log, self.mock_config, self.mock_http, self.mock_auth_cert
        )
        mock_request = MagicMock()
    ...
    async def test_file_metadata_get_single(self):
        controller = ControllerPuppetV3FileMetadata(
            self.log, self.mock_config, self.mock_http, self.mock_auth_cert
        )
        mock_request = MagicMock()
        mock_request.query_params = {}
