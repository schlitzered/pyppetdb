...
class TestApiV1HieraKeysUnit(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_key_model_exists_dynamic(self):
        ...
        self.mock_crud_dynamic.get.assert_called_once_with(_id="dynamic:test", fields=["id"])
    ...
    async def test_key_model_exists_static(self):
        ...
        self.mock_crud_static.get.assert_called_once_with(_id="static:test", fields=["id"])
    ...
    async def test_create_key(self):
        ...
        await self.controller.create(request=mock_request, data=data, key_id="key1", fields=set())
    ...
    async def test_update_key_with_validation(self):
        self.mock_authorize.require_admin = AsyncMock()
        # Mock current key
        self.mock_crud_keys.get = AsyncMock(return_value=MagicMock(key_model_id="dynamic:old"))
        ...
        mock_request = MagicMock()
        await self.controller.update(request=mock_request, data=data, key_id="key1", fields=set())
