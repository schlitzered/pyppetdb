...
class TestCrudHieraKeyModelsDynamicUnit(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        with patch("pyppetdb.crud.hiera_key_models_dynamic.CrudHieraModelsDynamicAdapter") as mock_adapter_class:
            self.mock_adapter = mock_adapter_class.return_value
            self.crud = CrudHieraKeyModelsDynamic(self.mock_config, self.log, self.mock_coll, self.mock_pyhiera)
...
class TestCrudHieraModelsDynamicAdapterUnit(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.adapter = CrudHieraModelsDynamicAdapter(self.log, self.mock_coll, self.mock_pyhiera)

    async def test_handle_change_insert(self):
        change = {
            "operationType": "insert",
            "documentKey": {"_id": "doc1"},
            "fullDocument": {"id": "dynamic:test", "model": {"type": "string"}, "description": "desc"}
        }
