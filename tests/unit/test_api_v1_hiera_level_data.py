...
class TestApiV1HieraLevelDataUnit(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_create_level_data_validation(self):
        ...
        self.mock_crud_keys.get = AsyncMock(return_value=MagicMock(key_model_id="static:test"))
        ...
        self.mock_crud_level_data.create = AsyncMock(return_value=MagicMock(facts={"os": "linux"}))
        ...
        await self.controller.create(
            request=mock_request, data=data, level_id="l1", data_id="d1", key_id="k1", fields=set()
        )
        
        mock_model_type.return_value.validate.assert_called_once_with({"foo": "bar"})
        self.mock_crud_level_data.create.assert_called_once()
        self.mock_cache.delete_by_key_and_facts.assert_called_once_with(key_id="k1", facts={"os": "linux"})
    ...
    async def test_create_missing_key_model(self):
        self.mock_authorize.require_admin = AsyncMock()
        self.mock_crud_keys.get = AsyncMock(return_value=MagicMock(key_model_id="static:unknown"))
        self.mock_crud_levels.get = AsyncMock(return_value=MagicMock(priority=10))
    ...
    async def test_delete_level_data_clears_cache(self):
        self.mock_authorize.require_admin = AsyncMock()
        self.mock_crud_level_data.get = AsyncMock(return_value=MagicMock(facts={"env": "prod"}))
        self.mock_crud_level_data.delete = AsyncMock()
        self.mock_cache.delete_by_key_and_facts = AsyncMock()
        
        mock_request = MagicMock()
        await self.controller.delete(request=mock_request, level_id="l1", data_id="d1", key_id="k1")
        
        self.mock_crud_level_data.delete.assert_called_once()
        self.mock_cache.delete_by_key_and_facts.assert_called_once_with(key_id="k1", facts={"env": "prod"})
    ...
    async def test_get(self):
        ...
        mock_request = MagicMock()
        await self.controller.get(request=mock_request, level_id="l1", data_id="d1", key_id="k1", fields=set())
    ...
    async def test_update(self):
        ...
        self.mock_crud_level_data.get = AsyncMock(return_value=MagicMock(facts={"os": "linux"}))
        ...
        mock_model_type.return_value.validate.assert_called_once_with({"foo": "new-bar"})
