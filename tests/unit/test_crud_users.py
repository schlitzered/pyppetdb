...
class TestCrudUsersUnit(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.crud = CrudUsers(
            self.mock_config, self.log, self.mock_coll, self.mock_crud_ldap
        )
    ...
    async def test_create_internal(self):
        with patch.object(CrudUsers, "_ph") as mock_ph:
            ...
            payload = UserPost(
                name="User One", email="user1@ex.com", password="password123"
            )
    ...
    async def test_create_external(self):
        ...
        result = await self.crud.create_external(
            _id="user1", payload=payload, fields=[], backend="ldap"
        )
    ...
    async def test_check_credentials_ldap_success(self):
        self.mock_coll.find_one = AsyncMock(
            return_value={"id": "user1", "backend": "ldap"}
        )
    ...
    async def test_update_internal_password(self):
        with patch.object(CrudUsers, "_ph") as mock_ph:
            ...
            self.crud._update = AsyncMock(
                return_value={"id": "user1", "name": "New Name"}
            )
