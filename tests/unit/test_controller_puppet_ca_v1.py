...
class TestControllerPuppetCaV1CAUnit(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.controller = ControllerPuppetCaV1CA(
            self.log, self.mock_config, self.mock_crud_authorities, 
            self.mock_crud_spaces, self.mock_crud_certificates, 
            self.mock_ca_service, self.mock_auth_cert
        )
    ...
    async def test_get_certificate_ca_not_found(self):
        self.mock_ca_service.get_certificate_chain.side_effect = ResourceNotFound("Space")
        mock_request = MagicMock()
        with self.assertRaises(HTTPException) as cm:
