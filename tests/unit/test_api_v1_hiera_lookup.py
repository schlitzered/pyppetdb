...
class TestApiV1HieraLookupUnit(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_lookup_cached(self):
        ...
        self.mock_cache.get_cached = AsyncMock(return_value={"result": {"data": "cached-value"}})
        
        mock_request = MagicMock()
        result = await self.controller.lookup(request=mock_request, key_id="mykey", fact={"os:linux"})
    ...
    async def test_lookup_no_cache(self):
        ...
        mock_request = MagicMock()
        result = await self.controller.lookup(request=mock_request, key_id="mykey", fact=set(), merge=False)
