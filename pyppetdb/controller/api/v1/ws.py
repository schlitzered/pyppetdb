...
class ControllerApiV1Ws:
    ...
    async def websocket_endpoint(
        ...
        await self._conns.connect(websocket=websocket)
        try:
            while True:
                data = await websocket.receive_text()
                await self._conns.send_personal_message(
                    message=f"You wrote: {data}",
                    websocket=websocket,
                )
                await self._conns.broadcast(message=f"Client #{client_id} says: {data}")
        except WebSocketDisconnect:
            self._conns.disconnect(websocket=websocket)
            await self._conns.broadcast(message=f"Client #{client_id} left the chat")
        except Exception:
            self._conns.disconnect(websocket=websocket)
        ...
        try:
            ...
            while True:
                ...
                if msg.msg_type == "subscribe_job_logs":
                    job_run_id = msg.msg_body.id
                    self._log.info(
                        msg=f"WS inter_api: {cn} subscribing to logs for {job_run_id}"
                    )
                    await self._ws_manager.subscribe(
                        websocket=websocket,
                        job_run_id=job_run_id,
                    ...
                    self._log.info(
                        msg=f"WS inter_api: {cn} requesting log chunks for {job_run_id}"
                    )
                    ...
                    job_run_id = msg.msg_body.job_run_id
                    request_id = msg.msg_body.request_id
                    chunk_id = msg.msg_body.chunk_id
                    self._log.info(
            ...
            self._log.info(
                msg=f"WS inter_api: connection disconnected from {cn if 'cn' in locals() else client_host}"
            )
            for job_run_id in subscriptions:
                await self._ws_manager.unsubscribe(
                    websocket=websocket,
