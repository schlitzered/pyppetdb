...
class PuppetCAStressTest:
    ...
    def _load_puppet_certs(self):
        ...
        key_path = os.path.join(self.puppet_ssl_path, "private_keys", f"{self.fqdn}.pem")
        ...
        while self.running:
            ...
            try:
                ...
                if resp.status_code not in (200, 201, 204):
                    logger.error(f"Worker {worker_id}: Failed to submit CSR for {cn}: {resp.status_code} {resp.text}")
                ...
                if resp.status_code not in (200, 204):
                    logger.error(f"Worker {worker_id}: Failed to revoke {cn}: {resp.status_code} {resp.text}")
                else:
    ...
    async def cleanup(self):
        async with self._lock:
            if not self.active_certs:
                return
            logger.info(f"Cleaning up {len(self.active_certs)} remaining active certificates...")
            ...
            for cn in list(self.active_certs):
                try:
                    ...
                    if resp.status_code in (200, 204):
                        ...
                        logger.error(f"Cleanup: Failed to revoke {cn}: {resp.status_code}")
                except Exception as e:
...
async def main():
    ...
    parser.add_argument("--url", required=True, help="Base URL of PyppetDB (e.g. https://localhost:8000)")
    parser.add_argument("--parallel", type=int, default=5, help="Number of parallel workers")
    parser.add_argument("--ssl-path", default="/etc/puppetlabs/puppet/ssl", help="Path to Puppet SSL directory")
