...
async def run_client(base_url, insecure=False):
    ...
    if not node_id:
        print("Failed to determine node_id from certificate CN. Using FQDN as fallback.")
    ...
    try:
        ...
        async with websockets.connect(url, ssl=ssl_context, open_timeout=10) as websocket:
